...
\alignauthor {Jay Lofstead\footnotemark[1], Quincey Koziol\footnotemark[3], John Bent\footnotemark[4], Eric Barton\footnotemark[5]}\\
...
platforms.
%Each layer of the proposed stack is discussed in some detail along
%with cross-cutting topics that impact multiple layers, such as transactions and
%metadata management.
%We also describe the proposed features to better support
...
the decisions.
%For example, extensive availability of NVRAM or SSDs on all of
...
%This layer incorporates the necessary
...
%the shared, persistent storage array.
...
storage array.
%Think of this layer as the physical disk interface layer. In
...
%The additional functionality, such as
...
%the new API.
%The additions comprise (Figure~\ref{fig:vol-arch}):
%
%\begin{enumerate}
...
%As shown in Figure~\ref{fig:vol-arch} it is implemented
...
%\vspace{-0.20in}
%\caption{VOL Abstration Architecture}
%\label{fig:vol-arch}
%\vspace{-0.10in}
%\end{figure}
%
...
%This mapping is shown in Table~\ref{tab:mapping}
...
%A current trend mixing MPI with
...
%themselves~\cite{lofstead:2010:io-variability}.
...
%the underlying storage array better than simple forwarding gateways.
For this
...
layer.
%With the additional desire to support using compute node resources for
...
%may be striped across multiple IO nodes.  Given this context, the
...
%transaction mechanism can work very well.
...
%The thought of using this mechanism to store
